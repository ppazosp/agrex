...
function ShapeItem({ shape, label }: { shape: React.ReactNode; label: string }) {
  return <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>{shape}<span style={{ opacity: 0.6 }}>{label}</span></div>
}
  ...
  return (
    <>
      <aside ref={containerRef} style={{
        ...
        border: '1px solid var(--agrex-node-border)', display: 'flex', flexDirection: 'column',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: compact ? '4px 8px' : '4px 12px', fontSize: compact ? 10 : 12, color: 'var(--agrex-fg)' }}>
          <Section title="Shapes">
            <ShapeItem shape={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1.5" y="1.5" width="11" height="11" rx="2.5" stroke="var(--agrex-node-border)" strokeWidth="1.5" /></svg>} label="Agent" />
            <ShapeItem shape={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="5.5" stroke="var(--agrex-node-border)" strokeWidth="1.5" /></svg>} label="Tool" />
            <ShapeItem shape={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M5.8 1.2Q7 0.5 8.2 1.2L11.6 3.1Q12.8 3.85 12.8 5.25V8.75Q12.8 10.15 11.6 10.9L8.2 12.8Q7 13.5 5.8 12.8L2.4 10.9Q1.2 10.15 1.2 8.75V5.25Q1.2 3.85 2.4 3.1Z" stroke="var(--agrex-node-border)" strokeWidth="1.5" /></svg>} label="File" />
          </Section>
          <Section title="Data Flow" defaultOpen={!compact}>
            <ShapeItem shape={<div style={{ width: 20, height: 0, borderTop: '1.5px solid var(--agrex-edge-spawn)' }} />} label="Spawn" />
            <ShapeItem shape={<div style={{ width: 20, height: 0, borderTop: '1.5px solid var(--agrex-edge-write)' }} />} label="Write" />
            <ShapeItem shape={<div style={{ width: 20, height: 0, borderTop: '1.5px solid var(--agrex-edge-read)' }} />} label="Read" />
          </Section>
          <Section title="Status">
            <ShapeItem shape={<div style={{ width: 10, height: 10, borderRadius: '50%', border: '2px solid var(--agrex-status-running)' }} />} label="Running" />

...
interface FileNodeData { label: string; status: NodeStatus; icon?: ComponentType<{ size: number }>; [key: string]: unknown }
      ...
      <div style={{ position: 'relative', width: S, height: H }}>
        ...
        <svg width={S} height={H} viewBox={`0 0 ${S} ${H}`} style={{
          ...
          <path d="M 19.6,3.4 Q 24,1 28.4,3.4 L 42.6,11.3 Q 47,13.75 47,18.75 L 47,36.25 Q 47,41.25 42.6,43.7 L 28.4,51.6 Q 24,54 19.6,51.6 L 5.4,43.7 Q 1,41.25 1,36.25 L 1,18.75 Q 1,13.75 5.4,11.3 Z"
            fill="var(--agrex-node-fill)" stroke={border} strokeWidth="2" />
        </svg>
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--agrex-node-icon)' }}>
          {Icon
            ? <Icon size={16} />
